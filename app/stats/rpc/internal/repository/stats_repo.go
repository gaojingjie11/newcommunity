...
func (r *StatsRepo) CommunityOverview() (model.CommunityOverview, error) {
	...
	if tx := r.db.Table("workorders").Where("type = ?", "repair").Count(&repairCount); tx.Error != nil {
		return ov, tx.Error
	}
	ov.RepairCount = repairCount

	var complaintCount int64
	if tx := r.db.Table("workorders").Where("type = ?", "complaint").Count(&complaintCount); tx.Error != nil {
		return ov, tx.Error
	}
	ov.ComplaintCount = complaintCount

	if tx := r.db.Table("property_fees").Count(&ov.FeeCount); tx.Error != nil {
		return ov, tx.Error
	}
	if tx := r.db.Table("property_fees").Where("status = 1").Count(&ov.FeePaidCount); tx.Error != nil {
		return ov, tx.Error
	}
