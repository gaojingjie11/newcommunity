package logic

import (
	"context"

	"smartcommunity-microservices/app/mall/rpc/internal/svc"
	"smartcommunity-microservices/app/mall/rpc/types/mall"

	"github.com/zeromicro/go-zero/core/logx"
)

type UpdateStoreProductStatusLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewUpdateStoreProductStatusLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UpdateStoreProductStatusLogic {
	return &UpdateStoreProductStatusLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
		Logger: logx.WithContext(ctx),
	}
}

func (l *UpdateStoreProductStatusLogic) UpdateStoreProductStatus(in *mall.UpdateStoreProductStatusReq) (*mall.BaseResp, error) {
	err := l.svcCtx.StoreSvc.UpdateProductStatus(in.StoreId, in.ProductId, int(in.Status))
	if err != nil {
		return &mall.BaseResp{Code: 500, Message: err.Error()}, nil
	}
	return &mall.BaseResp{Code: 0, Message: "success"}, nil
}
